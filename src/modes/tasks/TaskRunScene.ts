...
<div class="bm-shell bm-wide meadow-shell task-shell">
  <header class="meadow-header">
    <div>
      ...
      <h1 data-ui="taskName">Task</h1>
      <p class="bm-tagline">A short meadow objective.</p>
    </div>
  </header>
  <div class="meadow-bar">
    ...
    <button type="button" class="bm-btn ghost" data-ui="back">Tasks</button>
  </div>
  <div class="meadow-field">
    <canvas data-ui="canvas" width="960" height="540" aria-label="Moon Task"></canvas>
    <div class="meadow-overlay" data-ui="overlay">
      <div class="meadow-card">
        <div class="emoji">🌙</div>
        <h2 data-ui="title">Task</h2>
        <p data-ui="message">Ready?</p>
        <div class="meadow-end-actions">
          <button type="button" class="bm-btn warm" data-ui="play">Let's hop →</button>
          <button type="button" class="bm-btn ghost" data-ui="toSelect" hidden>Task list</button>
        </div>
      </div>
    </div>
    <div class="meadow-overlay meadow-pause" data-ui="pausePanel" hidden>
      <div class="meadow-card">
        ...
        <div class="meadow-pause-actions">
          ...
          <button type="button" class="bm-btn ghost" data-ui="quitSelect">Quit to Tasks</button>
        </div>
      </div>
    </div>
  </div>
  ...
  create(): void {
    ...
    this.runtime = new TaskRuntime(
      ...
      },
      {
        ...
        onQuitToSelect: () => this.scene.start("TaskSelect"),
      },
      task,
    )
