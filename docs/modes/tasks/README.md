...
its own Phaser scene ([bunny-jump.md](bunny-jump.md)).
...
| Bunny Jump | Climb one-way pads to a moon disc. Keep going for a local best height | Phaser FIT 1920x1080 (`BunnyJump`) | live ([I5](../../iterations/i5-bunny-jump.md)) |
...
upward-only camera. Task Select launches `BunnyJump` when `kind` is
`bunny_jump`. `contentFlags.tasks` still gates the whole mode.

Wins write `tasksCompleted` and a pantry reward. Bunny Jump also writes
`progress.bunnyJumpBest` (meters), same idea as `endlessBest`.
