...
| Name | Role | Lore |
| --- | --- | --- |
| Mei | Player. Eldest kit. Fast, brave. Talks to the Moon | invented |
| Yue ("Moon") | Youngest kit. Glows faintly. Goes missing | invented |
...
An osmanthus blossom from [Wu Gang's moon tree](LORE.md#wu-gang-吳剛) drifts down glowing. Yue chases it. The [Crane Envoy](LORE.md#cranes-as-immortal-messengers), sent to bring the blossom back, mistakes the glowing kit for it and carries Yue toward the moon.
...
> The path is soft underfoot. Keep hopping, little one.
...
> She is safe above the clouds. Climb when the water turns silver.
...
volume while he is not settled to cancel the fight and reset Han hearts. Mooncakes spawn on
court pads only. Spirit HUD only while
...
After Han is beaten, Customize unlocks Mei's moon helmet (cosmetic only).
...
- Dormant water is a still dusk puddle. First touch of that puddle awakens its reflection (silver moon-rim) and sets the station checkpoint to that pool for this run. A station may have several Moon Pools. Each has its own Chang'e line.
- Chang'e's line runs in the HUD ticker at the top center of the playfield. Play does not pause. DialogueOverlay is not used for the pool. A pool with an overlay `line` uses that text. Otherwise the station i18n key.
